...
def create_mat(y_pred,Y_train):
    vari_positif=sum( [1 for i in range(len(y_pred)) if Y_train[i]==0 and y_pred[i]==0 ] )
    faux_positif=sum( [1 for i in range(len(y_pred)) if Y_train[i]==1 and y_pred[i]==0 ] )
    faux_negatif=sum( [1 for i in range(len(y_pred)) if Y_train[i]==0 and y_pred[i]==1 ] )
    vrai_negatif=sum( [1 for i in range(len(y_pred)) if Y_train[i]==1 and y_pred[i]==1 ] )
    mat_confusion=np.array([[vari_positif,faux_positif],[faux_negatif,vrai_negatif]])
    Taux=(faux_positif+faux_negatif)*100/len(y_pred)
    return mat_confusion,Taux